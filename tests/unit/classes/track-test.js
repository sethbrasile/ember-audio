...
test('position.raw works', function(assert) {
  ...
  assert.equal(result.get('position.raw'), 65);
});

test('position.string works', function(assert) {
  let result = Track.create({ audioContext, startOffset: 65 });

  assert.equal(result.get('position.string'), '01:05');
  ...
  assert.equal(result.get('position.string'), '10:00');

  result.set('startOffset', 6001);
  assert.equal(result.get('position.string'), '100:01');

  result.set('startOffset', 6012);
  assert.equal(result.get('position.string'), '100:12');
});
...
test('percentPlayed works', function(assert) {
  ...
  assert.equal(result.get('percentPlayed'), 70);
});
